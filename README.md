...
SPA(Single Page Applicaton)는 서버로부터 완전한 새로운 페이지를 불러오지 않고 현재의 페이지를 동적으로 다시 작성함으로써 사용자와 소통하는 웹 애플리케이션이나 웹사이트이다.     
   
MPA(Multi Page Application) 페이지가 요청될 때마다, 서버로부터 전체 페이지를 받아오고, 이후에 데이터의 수정이나 조회가 필요할 때 완전히 다른 페이지로 이동한다.   
